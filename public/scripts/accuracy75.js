...
var dataToPostScenario2 = [];

accuracy75 = getTime() 

dataToPostScenario2.push( {participantID, accuracy75});
...
setTimeout(() => {
  ...
  dataToPostScenario2.push({participantID, InterferenceLog});
  setTimeout(() => {
...
function cb(droneIndex) {
  ...
  selectedDroneEmissions_p.innerText =
    ...
    if (drones[selectedDroneIndex] instanceof missclassifiedHostileDrone){
      ...
      console.log("fake hostile drone")
    ...
    dataToPostScenario2.push({participantID, droneID, timestampDroneClicked, AutoclassifierHostilityLeveL, droneClass });
